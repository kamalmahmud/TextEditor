...
public class EfficientDocument extends Document {
	...
	private void processText()
	{
		...
		for (int i = 0; i< tokens.size(); i++) {
			...
			if (isWord(tokens.get(i))) {
				...
				//Special case when last sentence does not end with punctuation.
				if (i == (tokens.size()-1)) {
					numSentences++;
				}
			}
			else {
				// When token is not word, it's punctuation, i.e. end of a sentence.
				numSentences++;
			}
	...
	public static void main(String[] args)
	{
	    ...
	    testCase(new EfficientDocument("This is a test.  How many???  "
                ...
                16, 13, 5);
		testCase(new EfficientDocument("For instance, in some styles of writing, particularly journalistic styles, a paragraph can be just one sentence long."),30,18,1);
